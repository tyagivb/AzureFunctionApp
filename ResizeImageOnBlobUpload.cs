...
namespace AzureTangyFunc
{
    public class ResizeImageOnBlobUpload
    {
        ...
        public async Task Run([BlobTrigger("functionsalesrep/{name}", Connection = "AzureWebJobsStorage")]Stream myBlob,
            [Blob("functionsalesrep-sm/{name}",FileAccess.Write)] Stream myBlobOutput,
            string name, ILogger log)
        {
            log.LogInformation($"C# Blob trigger function Processed blob\n Name:{name} \n Size: {myBlob.Length} Bytes");
             IImageFormat imageFormat = await Image.DetectFormatAsync(myBlob);
            using Image<Rgba32> input = Image.Load<Rgba32>(myBlob);
            
            input.Mutate(x => x.Resize(300, 200));
            input.Save(myBlobOutput, imageFormat);
        }
    }
}
